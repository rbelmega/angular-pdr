...
factories.listOfTopic = function() {
	...
	var items = [
		...
		{
			'id': 2,
			'name': "Окремі питання організації дорожнього руху, що потребують узгодження з Державтоінспекцією"
		},
...
factories.itemsFactory = function($http, $q) {
	var factory = {};

	factory.getItemById = function(id, callback) {

		!isNaN(id) && $http.get('data/topics/' + id + '.txt').
...
factories.finesFactory = function($http, $q) {

	var factory = {};
	var deferred = $q.defer();

	factory.getFines = function() {
		$http.get('data/fines/fines.json').
			success(function(data){
				deferred.resolve(data);
			});
